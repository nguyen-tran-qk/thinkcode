(function() {
  ...
  function lessonCtrl($scope, $rootScope, $state, $timeout, $filter, $q, Utils, UserService, WorkspaceService, CoursesService) {
    ...
    vm.getWorkspace = function(workspaceId) {
      WorkspaceService.getWorkspaceById(workspaceId, vm.$state.params.course_id, function(res) {
        ...
        if (res.status === 401) {
          ...
          $scope.showMessage('danger', 'Không tìm thấy bài học.');
        } else {
          $scope.showMessage('danger');
        }
      });
